...
uint16_t Party::atkOf(const PartyMon &m) const {
  return m.empty() ? 0 : personalityApply(calcStat(DEX_TBL[m.dex].bAtk, m.ivAtk, m.level, m.trAtk), persOf(m), PST_ATK);
}
uint16_t Party::defOf(const PartyMon &m) const {
  return m.empty() ? 0 : personalityApply(calcStat(DEX_TBL[m.dex].bDef, m.ivDef, m.level, m.trDef), persOf(m), PST_DEF);
}
...
uint16_t Party::spdOf(const PartyMon &m) const {
  return m.empty() ? 0 : personalityApply(calcStat(DEX_TBL[m.dex].bSpD, m.ivDef, m.level, m.trDef), persOf(m), PST_SPD);
}
