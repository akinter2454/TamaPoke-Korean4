  ...
  if (pet.endedKind != CER_NONE && !partyPick) {
    ...
    if (party.add(pet.endedMon) || party.boxAdd(pet.endedMon)) {
      snprintf(partyBannerName, sizeof(partyBannerName), "%s",
               pet.endedMon.nick[0] ? pet.endedMon.nick : localizedSpeciesName(pet.endedMon.dex));
...
void renderMonSheet(const PartyMon &m, bool fromBox) {
  ...
  snprintf(head, sizeof(head), "%s%s Lv.%u", m.shiny ? "*" : "",
           m.nick[0] ? m.nick : localizedSpeciesName(m.dex), (unsigned)m.level);
  gfx->setTextColor(RGB565_BLACK);
  uiSetTextSize(2);
  uiSetCursor(CX - uiTextHalfWidth(head, 2), 40);
  gfx->print(head);
  char ty[24];
  if (d.type2 == T_NONE) snprintf(ty, sizeof(ty), "%s", localizedTypeName(d.type1));
  else snprintf(ty, sizeof(ty), "%s/%s", localizedTypeName(d.type1), localizedTypeName(d.type2));
  ...
  if (releaseConfirm) {
    char q[96];
    snprintf(q, sizeof(q), T(S_RELEASE_FMT), m.nick[0] ? m.nick : localizedSpeciesName(m.dex));
...
void drawCareTabs() {
  ...
  for (uint8_t i = 0; i < CARE_SLOT_COUNT; i++) {
    ...
    if (dex >= 1 && dex <= DEX_COUNT) edge = typeColor(DEX_TBL[dex].type1);
...
static uint8_t currentSceneType() {
  if (pet.isEgg() || pet.speciesId < 1 || pet.speciesId > DEX_COUNT) return T_NORMAL;
  uint8_t t = DEX_TBL[pet.speciesId].type1;
...
void render() {
  ...
  if (pet.ceremony) {
    ...
    const char *msg = (pet.ceremony == CER_FAREWELL) ? T(S_FAREWELL)
                      : (pet.ceremony == CER_RUNAWAY) ? T(S_RUNAWAY)
                                                      : T(S_GOODBYE);
    drawHeader(localizedSpeciesName(pet.speciesId), scenePokemonNameColor(currentSceneType(), gNight), msg);
    drawCeremony();
    gfx->flush();
    return;
  }
  ...
  if (pet.isEgg()) {
    ...
    const DexEntry &d = DEX_TBL[pet.speciesId];
    char name[48];
    const char *base = pet.nick[0] ? pet.nick : localizedSpeciesName(pet.speciesId);
  ...
  if (confirmUntil) {
    if (millis() > confirmUntil) {
      ...
      char q[96];
      snprintf(q, sizeof(q), T(S_RELEASE_FMT), localizedSpeciesName(pet.speciesId));
      gfx->setTextColor(UI_INK);
      uiSetTextSize(2);
...
void gameTap(int16_t x, int16_t y) {
  (void)x; (void)y;
  if (gameOverUntil) { gameOpen = false; return; }
  uint32_t now = millis();
...
void renderCardProfile() {
  const DexEntry &d = DEX_TBL[pet.speciesId];
  const char *nm = pet.nick[0] ? pet.nick : localizedSpeciesName(pet.speciesId);
  char head[48];
  ...
  if (pet.nick[0]) {  // especie real bajo el apodo
    gfx->setTextColor(RGB565_BLACK);
    uiSetTextSize(2);
    const char *species = localizedSpeciesName(pet.speciesId);
...
void renderCardStats() {
  ...
  char ty[24];
  if (de.type2 == T_NONE) snprintf(ty, sizeof(ty), "%s", localizedTypeName(de.type1));
  else snprintf(ty, sizeof(ty), "%s/%s", localizedTypeName(de.type1), localizedTypeName(de.type2));
...
static const char *btlDisplayName(const Combatant &c) {
  ...
  return localizedSpeciesName(c.dex);
}
...
void spdTap(int16_t x, int16_t y) {
  if (spdOverUntil) { spdOpen = false; return; }
...
static void drawPickCell(uint8_t n, int x, int y, uint8_t capLvl) {
  ...
  if (n == 0) {
    ...
    nm = pet.nick[0] ? pet.nick : localizedSpeciesName(dex);
  } else {
    const PartyMon &m = party.slots[n - 1];
    dex = m.dex; lvl = m.level; shiny = m.shiny;
    nm = m.nick[0] ? m.nick : localizedSpeciesName(dex);
  }
  ...
  gfx->print(localizedTypeName(d.type1));
  if (d.type2 != T_NONE) {
    uiSetCursor(x + 54, y + 60);
    gfx->print(localizedTypeName(d.type2));
  }
  if (on) {
...
void renderPick() {
  gfx->fillScreen(RGB565_BLACK);
  gfx->fillCircle(CX, CY, 231, UI_BG_DAY);
  uint8_t cap = (pickTrainer == PICK_LAN) ? TRAINER_TEAM_MAX : squadCapForRegion(pickRegion, pickTrainer, pickHard);
...
static void drawEggRegion() {
  char l[24];
  snprintf(l, sizeof(l), "%s >", localizedRegionName(pet.region));
...
static void renderRegionPick(uint8_t mode) {
  ...
  for (uint8_t row = 0; row < RPICK_PER_PAGE; row++) {
    ...
    const char *nm = localizedRegionName(forGyms ? gymRegionDexRegion(i) : i);
...
void renderCardProgress() {
  ...
  if (!dexHasEvolution(pet.speciesId)) {
    evo = T(S_FINAL_FORM);
  } else {
    ...
    int needed = effectiveEvolutionLevel(pet.speciesId, pet.careMistakes, pet.evoPenalty());
    if (pet.level() >= needed) {
...
static bool combatantFromCareSlot(uint8_t slot, Combatant &c, uint32_t nowEpoch) {
  ...
  c.base[SI_SPE] = personalityApply(careCalcStat(DEX_TBL[m.speciesId].bSpe, m.ivSpe, lvl, m.trSpe), pers, PST_SPE);
  memcpy(c.moves, m.moves, sizeof(c.moves));
  c.shiny = m.shiny != 0;
  const char *nm = m.nick[0] ? m.nick : DEX_TBL[m.speciesId].name;
...
void startBossBattle() {
  if (bossReadyCount() < 3 || pet.ceremony != CER_NONE) { sfxPlay(SFX_DENY); return; }
  uint32_t e = rtcEpoch();
  if (!e) e = pet.lastSeenEpoch;
  btlSquadN = 0; btlSquadAt = 0; btlPetIn = true;
  uint8_t maxLv = 1;
  for (uint8_t i = 0; i < CARE_SLOT_COUNT; i++) {
    Combatant c;
    if (!combatantFromCareSlot(i, c, e)) continue;
...
void renderExplore() {
  ...
  if (extras.explorationReady()) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (!pet.isEgg() && DEX_TBL[pet.speciesId].type1 == t) { gfx->setTextColor(UI_BAR_OK); uiSetTextSize(1); uiSetCursor(318, y + 12); gfx->print("보너스"); }
    }
...
void renderBoss() {
  uint8_t t=extras.bossType(pet); uint8_t ready=bossReadyCount();
  gfx->fillScreen(RGB565_BLACK); gfx->fillCircle(CX,CY,231,UI_BG_DAY);
  ...
  for(uint8_t i=0;i<3;i++){int y=214;int x=70+i*110;int d=careSlotDex(i);uint16_t c=d>=1?typeColor(DEX_TBL[d].type1):UI_TRACK;gfx->fillRoundRect(x,y,96,62,12,UI_WHITE);gfx->drawRoundRect(x,y,96,62,12,c);char sl[16];snprintf(sl,sizeof(sl),"슬롯 %u",i+1);gfx->setTextColor(c);uiSetTextSize(1);uiSetCursor(x+10,y+8);gfx->print(sl);gfx->setTextColor(UI_INK);if(d>=1){uiSetTextSize(1);uiSetCursor(x+8,y+30);gfx->print(localizedSpeciesName(d));}else{uiSetTextSize(1);uiSetCursor(x+18,y+30);gfx->print("준비 필요");}}
  gfx->fillRoundRect(104,302,258,62,16,ready==3?UI_BAR_BAD:UI_TRACK);gfx->setTextColor(UI_WHITE);uiSetTextSize(3);const char* go=ready==3?"보스 도전":"3마리 필요";uiSetCursor(CX-uiTextHalfWidth(go,3),321);gfx->print(go);
  gfx->setTextColor(UI_TRACK);uiSetTextSize(2);uiSetCursor(CX-uiTextHalfWidth("뒤로",2),405);gfx->print("뒤로");gfx->flush();
}

void bossTap(int16_t x,int16_t y){if(x>=104&&x<=362&&y>=302&&y<=364){startBossBattle();return;}if(y>390){bossOpen=false;adventureOpen=true;}}
...
void drawMenu() {
  ...
  for (int i = 0; i < MENU_ROWS; i++) {
    ...
    bool dead = (i == 5 && !pet.canRetireNow());   // an egg or a companion
...
void drawPartySlot(int i, int x, int y) {
  ...
  const DexEntry &d = DEX_TBL[m.dex];
  const char *nm = m.nick[0] ? m.nick : localizedSpeciesName(m.dex);
  gfx->setTextColor(RGB565_BLACK);
  uiSetTextSize(1);
...
void renderParty() {
  ...
  if (boxSel) {
    ...
    snprintf(sw, sizeof(sw), T(S_BOX_SWAP),
             b.nick[0] ? b.nick : localizedSpeciesName(b.dex));
    gfx->setTextColor(UI_BAR_WARN);
    uiSetTextSize(1);
