# TamaPoke KO v3.62.7 — 새 ChatGPT 계정 인수인계 문서

## 0. 이 문서를 먼저 읽을 것

이 프로젝트는 Waveshare 원형 AMOLED용 TamaPoke 한국어판을 장기간 수정해 온 프로젝트다. 새 계정/새 대화에서는 **이 HANDOFF 문서와 v3.62.7 전체 GitHub ZIP을 함께 업로드한 뒤 작업을 시작**한다. 과거 v3.2x~v3.60 실험판으로 되돌리지 말고, 반드시 **v3.62.7 SingleRelease FullSD + BasePackGuard + Canonical Forms + Regional Evolution + Reliable SD Sync + Sprite Audit + Mega36**를 최신 기준선으로 사용한다.
...
- PMDCollab의 표현용 리소스 슬롯: **AltColor/Alt Colour, Alternate, Cutscene, Beta**. 이것들은 같은 포켓몬의 recolor/대체 스프라이트이므로 별도 species/도감/진화형으로 만들지 않는다.
...
- 과거 lock에 들어간 AltColor/Alternate/Cutscene/Beta ID는 v3.62.5 sync에서 제거하지만 `next_form_id`는 낮추지 않는다. **퇴역 ID는 다른 포켓몬에 재사용하지 않는다.**
- 생성된 `DEX_RETIRED_VARIANT_BASE`로 과거 잘못된 세이브 ID를 정상 base/지역폼으로 migration한다.
...
## v3.62.2 Sprite audit / full sync
- 사진에서 팔데아 켄타로스 이름에 라프라스 sprite가 나온 문제는 1200+ form ID와 오래된 SD p<ID>.bin 불일치 가능성이 핵심이다.
- 모든 PMD path가 해당 NatDex owner 폴더로 시작하는지 Actions에서 전수 검사하고, Paldea Tauros는 0128/*만 허용한다.
- sprite-audit.json 및 sprite-mapping-report.txt를 Pages에 공개한다.
- 810+는 실제 TPK3 패킹 원본에서 preview PNG를 생성하며 설치기의 '리소스 검수' 탭에서 이름/ID/NatDex/PMD 경로와 함께 볼 수 있다.
- 설치기 '전체 스프라이트' 탭은 Kanto~Alola base pack 7개와 현재 added-all 팩을 순차 전송해 기존 SD를 현재 catalog_fingerprint와 맞춘다.
- 성공 Actions는 catalog_lock.json을 repo에 커밋하므로 이후 1200+ ID는 append-only로 고정한다.


## v3.62.3 Reliable WebSerial / microSD sprite sync
- 실제 기기에서 전체 동기화가 `mons/p001.bin`부터 또는 임의 파일에서 멈추는 현상을 수정했다.
- 평상시 게임은 `Serial.setTxTimeoutMs(0)`을 계속 사용하여 serial monitor가 없어도 렌더링이 멈추지 않는다.
- 단, 명시적인 `SDINFO` / `PUT` 명령 동안만 TX timeout을 1000ms로 올리고 `OK`, 각 2KiB 블록의 `#`, 마지막 `DONE`을 `Serial.flush()`하여 host가 반드시 받도록 한다.
- firmware `SDINFO` 응답은 `SDINFO OK proto=2 cardMB=...`이며 SD가 없으면 `SDINFO ERR no-card`이다.
- PUT은 각 `f.write()`의 실제 written byte 수를 검사하고, 데이터 수신 timeout은 8초다.
- 설치기는 연결 직후 SDINFO preflight를 통과해야 지역/추가/Mega36/전체 동기화 버튼을 활성화한다.
- WebSerial read는 pending read 하나만 유지하며 `OK/#`는 15초, `DONE`은 30초 대기한다. 실패 단계와 block 번호를 로그에 표시한다.
- 전송 실패 시 browser가 serial session을 닫는다. 부분 파일 수신 중 다음 `PUT` 명령이 raw data로 섞이는 것을 막기 위해 반드시 기기를 재부팅/재연결한 후 재시도한다.
- `.github/workflows/main.yml`에 남아 있던 중복 `PYMEGADELTA` heredoc launcher도 제거했다.
- `tools/verify_sd_serial_protocol.py`가 이 프로토콜 보호를 Actions에서 회귀검사한다.
- v3.62.2의 Sprite Audit / Full Sync / Paldea Tauros source guard와 Mega36, IV, 진화 상한, 메뉴/절전/체육관 수정은 모두 유지한다.


### v3.62.3 current SHA-256
```text
ff428bcef91027beb426c5cca132caada918794fcec687dda9622eedd5d973e5  TamaPoke.ino
02484395824d8881e6720899dbb9c247f39af02c51a2a514863cbbf83e14e9d7  sdmon.cpp
9f92bd8819091b219ef92425e0caa000d369b70a8ce133ac3885584a3ea9bab1  sdmon.h
38e2c6f6930600dbc7a4fb2dd9853ec1eec9efe607f78b021cc8bdf9dd457ed8  TamaPoke-KO-OneClick-Installer.html
21d6b591193475eedf43753af8a3a641df602827eef621b3b327871c35fac648  .github/workflows/main.yml
937e84acc80b2f42da742e630266ba3fc1bf0ff87d966114e64fade9cdc78ff0  tools/verify_sd_serial_protocol.py
332e88b8cab7a25c187113e7257c98020e56cb188b358a730b23c3b5ddb77a78  tools/sync_pmd_catalog.py
62ba48dea71501741b9ce6c7eaa6d24325b2fb86001b62fa5dbd3cc7af3c5af8  tools/pack_pmd_catalog.py
8f6a93ec86da91e1b351989bbd65efd7f22fdbfe07eaaef539ec06e7d7c4a231  tools/build_sprite_paks.py
```

## v3.62.4 Regional Form Evolution Routing Audit
- National Dex parent만 보고 extra edge를 만들던 구조를 수정했다. 지역폼 전용 새 종은 해당 지역폼의 독립 TamaPoke ID에서만 진화한다.
- Galar descendant guard: #862 Obstagoon, #863 Perrserker, #864 Cursola, #865 Sirfetch'd, #866 Mr. Rime, #867 Runerigus.
- Hisui descendant guard: #903 Sneasler, #904 Overqwil.
- #902 Basculegion의 source인 Basculin White는 PMDCollab label에 `Hisui`가 없으므로 special-case로 Hisui region/tag를 부여한다. 이로써 Hisui pool에서 hatch 가능하고 Basculegion으로 진화한다.
- 같은 문제 유형인 Paldea Wooper -> Clodsire도 guard한다.
- Alola fixed 18폼 direct evolution과 Pikachu/Exeggcute/Cubone의 3개 Alola branch는 sync 단계에서 고정 회귀검사한다.
- `tools/verify_regional_evolutions.py`는 생성된 `pmd_catalog.json`/`dex.h`를 읽어 Alola/Galar/Hisui(+Paldea guard) 52조건을 검사하고, 잘못된 normal-parent edge가 하나라도 있으면 Actions를 실패시킨다.
- `tools/regional_evolution_report.txt`는 Pages에 복사되고 설치기 하단 링크에서 직접 확인할 수 있다.
- v3.62.3 SDINFO proto=2 / reliable PUT ACK, v3.62.2 sprite audit/full sync, Mega36, IV, 진화 상한, 전원/메뉴/체육관 로직은 유지한다.

### v3.62.4 current SHA-256
```text
c5692da9bb08d1c70e4bdaece49e1eb2118d09b9f51ff05b029c3fd94976df0f  TamaPoke.ino
5391913558f9f58bdb23818d7f86edfead44c798bf8e481bb1b6fbb458b97999  TamaPoke-KO-OneClick-Installer.html
a2769668c7380c7affd71ef9a5b3183adb60ab2505442912250ab25b394ba458  .github/workflows/main.yml
02e2cbb1607c333d6a2d41d8fe67b21583e13c9964e8032a793c42b86280b538  tools/sync_pmd_catalog.py
6a0d10dd044c38bd92deaf3215389eb1eefa126e3a4753bd29af296ad4730245  tools/verify_regional_evolutions.py
5c0d8974486a6efec6bb703734d7ffa45bcc62976e5df5678dd18f2fdea74fa4  tools/verify_sd_serial_protocol.py
77fc7f057b9f99f47d6ff490cce4a1e9566f3bfe0aa0337252f19efbfc30c0c6  tools/pack_pmd_catalog.py
```


## v3.62.5 Canonical Forms / presentation-only resource cleanup
- 실제 사용자 저장소의 과거 `catalog_lock.json`에는 PMDCollab `Altcolor`, `Alternate`, `Galar_Alternate`, `Cutscene_Altcolor` 등이 공식 폼처럼 1200+ 독립 ID를 받아 있던 이력이 있다.
- 이 때문에 리자드가 정상 리자몽 대신 `리자몽 Altcolor`로 진화할 수 있었고, Mega Charizard X edge는 정상 리자몽 ID에만 연결되어 이후 메가진화가 끊길 수 있었다.
- PMDCollab 표현용 슬롯 `AltColor/Alt Colour`, `Alternate`, `Cutscene`, `Beta`는 이제 catalog/species/evolution/resource 생성에서 전면 제외한다. Shiny는 기존 `psNNN.bin` 체계로만 관리한다.
- `prune_presentation_lock()`이 기존 lock의 잘못된 항목을 퇴역시키되 `next_form_id`를 유지한다. 퇴역 ID는 다시 쓰지 않는다.
- Actions 생성 `dex.h`의 `DEX_RETIRED_VARIANT_BASE` / `canonicalizeRetiredVariant()`가 과거 세이브를 자동 복구한다. `pet.cpp`, `party.cpp`, `care_slots.cpp`가 로드 시 이를 적용하며 레벨/IV/닉네임 등 개체 데이터는 보존한다.
- plain AltColor/Alternate는 정상 National-Dex base로, `Galar_Alternate`처럼 지역 태그가 있는 표현용 항목은 해당 canonical 지역폼으로 복구한다.
- `verify_canonical_forms.py`가 active lock/catalog에 표현용 슬롯이 남는지, retired ID가 evolution edge에 들어가는지, retired ID가 재활성화되는지 검사한다. 리자몽은 non-Mega extra form이 0개이고 Mega Charizard X 1개 + Lv.70 edge가 있는지 별도 회귀검사한다.
- SD 전송 프로토콜은 `SDINFO OK proto=3`으로 올리고 `/mons/p*.bin`에만 허용되는 제한적 `DEL` 명령을 추가했다. `sprite-manifest.json` schema 4의 `retired_files`를 이용해 `added-all`/전체 스프라이트 동기화가 오래된 `pNNN.bin`/`psNNN.bin`을 먼저 삭제한다.
- v3.62.4의 Alola/Galar/Hisui/Paldea regional evolution audit는 그대로 유지한다. `Galar_Alternate` 같은 가짜 폼을 제거함으로써 regional form selector ambiguity도 함께 예방한다.
- 설치 순서: 기존 repo의 `tools/catalog_lock.json` 보존 → v3.62.5 전체본 덮어쓰기 → Actions 성공(정리된 lock 자동 커밋) → v3.62.5 펌웨어 설치/재부팅 → 전체 스프라이트 동기화 1회. microSD 포맷은 필요 없다.

### v3.62.5 current SHA-256
```text
382f8324a4bb343a6781eb39ffdf29070f85d028d4db21b2fb593d59f9bcde58  TamaPoke.ino
dcf604933c9fe3f2ada057be52847f5e7245f26eb4c1d1f38e4d28240e6b8d98  dex.h
ed73b1fecb3212ab4dcdc1f7f6f7d5cede0c83c32090dcf4c5d725a52c230348  pet.cpp
6e775db6485e8c85ac31a2ae5a2360104e172c938bcc7a5da26746908d999645  party.cpp
ac54644f1fe30640edd2dd016941453fe62a290b7d4d21c4a087704eb8484b69  care_slots.cpp
a773af1248578d57b5e6cc62ab6b10fcfb9d6319eb3be0100782ad5cea7a2e43  sdmon.cpp
e0ac67004498fef68bb975557d7282c439c3741707193f80229b1f7d1528aeba  sdmon.h
efbadada5ffed85789858dd07de5c4d9180d1ba07f393140c525dc4a130a4a19  TamaPoke-KO-OneClick-Installer.html
3675c4aad95cacc02b1c3329301e58490774f0450c7b5ef650d75d599ba92f34  .github/workflows/main.yml
b5008464588919dbb5edaba3f15ecf847dff1628873ad77e8e5d8985792b7261  tools/sync_pmd_catalog.py
73511564cdfb350aa2ffa8489457cd199b6d5da1cb6623b57c4c6ff2a44659fa  tools/pack_pmd_catalog.py
7f0a1632c44b189f6e7c97f685be51ecb88b5e6ec3c110b237b443a66a526194  tools/build_sprite_paks.py
d7758d95d623c32ce5f37c281dfe6009a4ec2aac7a9ee9a4f12877a25da2717c  tools/verify_sd_serial_protocol.py
6a0d10dd044c38bd92deaf3215389eb1eefa126e3a4753bd29af296ad4730245  tools/verify_regional_evolutions.py
8facc86674f9a32b5280f5795e6eaa63dc497b41633a388d83a7b8749b498014  tools/verify_canonical_forms.py
```


## v3.62.6 BasePackGuard

- 관동~알로라 legacy 기본 팩의 다운로드 source를 DylanPDao/TamaPoke의 검수된 immutable commit `7d5a2b3f4a4bee4f107cdae77ad13949dddfc615`로 고정했다. `main` 및 `releases/latest` fallback은 제거했다.
- 해당 upstream packer는 normal을 PMDCollab `sprite/NNNN` root, shiny를 `sprite/NNNN/0000/0001`에서 만든다. AltColor/Alternate sibling을 normal로 탐색하지 않는다.
- 브라우저는 기본 팩을 microSD로 보내기 전에 정확한 지역 NatDex 범위, `mons/pNNN.bin`/`psNNN.bin` 허용 형식, TPK2/TPK3 magic, 중복 파일을 검사한다.
- `tools/sync_pmd_catalog.py::base_sprite_path()`의 grouping-only fallback도 AltColor/Alternate/Cutscene/Beta, gimmick, 지역폼 sibling을 거부한다. 잘못된 child를 쓰는 것보다 base art를 disabled 처리하는 정책이다.
- Actions에서 `tools/verify_base_pack_policy.py`를 실행하고 Pages에 `base-pack-policy-report.txt`를 제공한다.
- v3.62.5의 canonical-form 정리, save migration, retired SD file deletion, 지역폼 진화, Mega36, reliable SD proto=3은 그대로 유지한다.


## v3.62.6 PC용 전체 microSD ZIP

- 사용자는 Web Serial로 모든 sprite를 매번 보내지 않아도 된다. GitHub Actions가 `TamaPoke-SD-Sprites-v3.62.6-Full.zip`을 만든다.
- ZIP 최상위에 `mons/`가 있고, 기존 SD의 `/mons`를 백업/삭제한 뒤 이 폴더를 SD 루트에 복사하면 된다. NVS 세이브는 건드리지 않는다.
- 기본 #001~#809는 BasePackGuard의 immutable commit에서 7개 TPAK을 다시 다운로드해 파일명/지역범위/TPK2/3 magic을 검증한 뒤 풀어 넣는다.
- 810+/공식 지역폼/Mega36은 같은 run의 `tools/sdcard/mons`에서 active catalog ID만 복사한다. retired presentation-only ID는 허용하지 않는다.
- ZIP 안에 `README-SD-KO.txt`, `BUNDLE-INFO.json`, `SHA256SUMS.txt`, `sprite-audit.json`, `sprite-manifest.json`, `PMDCOLLAB_CREDITS.txt`를 포함한다.
- Release tag `sprites-v3.62.6`에 ZIP asset을 `--clobber`로 게시하고 Pages의 `sprite-bundle.json`에 직접 다운로드 URL/SHA-256/크기를 기록한다. 설치기의 `PC용 전체 스프라이트 ZIP 다운로드` 버튼은 기기 연결 없이 이 asset을 내려받는다.
- 동일 ZIP은 Actions artifact에도 백업한다.


## v3.62.7 SingleRelease FullSD storage guard

- PC용 전체 스프라이트 ZIP의 **내용/검수 구조는 v3.62.6과 동일**하고, GitHub 저장 방식만 무료 플랜 친화적으로 바꾼 버전이다.
- `actions/upload-artifact@v4`에 대용량 전체 sprite ZIP을 백업하던 step은 삭제했다. GitHub Pages 배포용 artifact는 사이트 배포에 필요한 별도 작은 artifact이므로 그대로 유지한다.
- 이전 run에서 이미 남아 있는 `TamaPoke-vX.Y.Z-full-sd-sprites` 대용량 Actions Artifact는 `actions: write` 권한으로 REST API를 호출해 다음 성공 build에서 자동 삭제한다. 다른 Actions artifact는 이름 패턴이 다르므로 건드리지 않는다.
- 전체 sprite ZIP은 `sprites-current` Release 하나에만 저장한다. 공개 asset 이름은 항상 `TamaPoke-SD-Sprites-Current-Full.zip`이다.
- Actions run 내부에서는 버전 추적을 위해 `TamaPoke-SD-Sprites-v<FW_VERSION>-Full.zip`을 임시 생성하지만 runner가 끝나면 사라지며 repository/Actions artifact/Pages에는 그 대용량 파일을 저장하지 않는다.
- build 성공 시 `sprites-current` Release 제목/노트를 현재 펌웨어 버전으로 갱신하고 같은 asset을 `gh release upload --clobber`로 교체한다.
- 그 다음 과거 workflow가 자동 생성한 `sprites-vX.Y.Z` 패턴의 sprite Release만 삭제하고 tag도 정리한다. 일반 펌웨어 Release나 다른 tag는 건드리지 않는다.
- Pages의 `sprite-bundle.json`은 `release_tag=sprites-current`, 안정적인 download URL, 원래 build filename, SHA-256, catalog fingerprint를 제공한다. 설치기의 PC ZIP 버튼은 이 URL을 사용한다.
- `tools/verify_release_storage_policy.py`가 versioned sprite Release 정책이나 full-ZIP Actions artifact가 다시 들어오면 CI를 실패시킨다.
- GitHub CLI의 rolling asset 교체는 mutable Release가 전제다. repository에서 Immutable Releases를 별도로 활성화하면 asset 교체가 제한될 수 있으므로 기본 mutable Release 설정을 유지한다.
- 이 버전은 sprite 파일 내용/도감/진화/게임 밸런스를 바꾸지 않는다. v3.62.6에서 SD 전체 동기화를 끝냈다면 **v3.62.7 때문에 sprite를 다시 받을 필요는 없다.**

### v3.62.7 current SHA-256

```text
a401444b72b78841f1879a26cab83252b0fb7d2b284e163b864a0f5eaaddf299  TamaPoke.ino
367ae67b631619bf06e37658cf736653281c5a05f4c790c006ca7c6492fdd104  TamaPoke-KO-OneClick-Installer.html
c0402a7cca257c594e31403bd0b8a22bd7db7d1427eae920911397e8b2879ca9  .github/workflows/main.yml
ee637b6e8e2810da594cc83c5bc90b10bbee7b671df15f97f159242689dceb01  tools/build_full_sd_sprite_zip.py
bbddb03007c17d007bb935a3f5ea1aaedb7393179fa428c9da8b0145ed3f2e3a  tools/verify_release_storage_policy.py
46ac9f445d8d2b94af096791e5d4603dfd9e017dac70500c2890d17f2744f4a5  tools/verify_full_sd_bundle.py
9f21b4794f4baed794b2433c277636a829ba0a31eebc14e08c948691cb722646  tools/verify_sd_serial_protocol.py
38ccb6d3550d30b0407adc781f81027440cf97c8c9ab3b06602dc3ffbdbc844c  tools/verify_base_pack_policy.py
eabf85d5ca3550486654b0040caa05d748b3e2a7b342103d7ef62df9d79e72b2  tools/sync_pmd_catalog.py
fcbb2198911105faec60796743d6179a90467d1565c3d42315f98c1a3567f11c  tools/pack_pmd_catalog.py
5a1b6867193c5b3627976556768d8e8baeb3b0684ea800afc4fa5678418a6a13  PROJECT_STATE_v3.62.7.json
```
