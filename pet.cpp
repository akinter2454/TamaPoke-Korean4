...
void Pet::tick() {
  ...
  if (ageMinutes % 3 == 0) energy = clamp100(energy - 1);
  ...
  if (weight > 50 && ageMinutes % 3 == 0) energy = clamp100(energy - 1);
  if (weight > 0 && ageMinutes % 3 == 0) weight--;
...
uint16_t Pet::atkStat() const {
  ...
  return personalityApply(calcStat(DEX_TBL[speciesId].bAtk, ivAtk, level(), trAtk), p, PST_ATK);
}
uint16_t Pet::defStat() const {
  if (isEgg()) return 0;
  uint8_t p = personalityIdFor(speciesId, ivAtk, ivDef, ivSpe, ivHp);
  return personalityApply(calcStat(DEX_TBL[speciesId].bDef, ivDef, level(), trDef), p, PST_DEF);
}
uint16_t Pet::speStat() const {
  if (isEgg()) return 0;
  uint8_t p = personalityIdFor(speciesId, ivAtk, ivDef, ivSpe, ivHp);
  return personalityApply(calcStat(DEX_TBL[speciesId].bSpe, ivSpe, level(), trSpe), p, PST_SPE);
}
...
uint16_t Pet::spaStat() const {
  if (isEgg()) return 0;
  uint8_t p = personalityIdFor(speciesId, ivAtk, ivDef, ivSpe, ivHp);
  return personalityApply(calcStat(DEX_TBL[speciesId].bSpA, ivAtk, level(), trAtk), p, PST_SPA);
}
uint16_t Pet::spdStat() const {
  if (isEgg()) return 0;
  uint8_t p = personalityIdFor(speciesId, ivAtk, ivDef, ivSpe, ivHp);
  return personalityApply(calcStat(DEX_TBL[speciesId].bSpD, ivDef, level(), trDef), p, PST_SPD);
}
...
uint8_t Pet::playResult(uint16_t score) {
  ...
  uint8_t eCost = (uint8_t)min<uint16_t>(18, 5 + score / 6);
...
uint8_t Pet::trainStrength(uint16_t hits) {
  ...
  energy = dropTo(energy, 7, 8);    // v3.20: training is less exhausting
  fullness = dropTo(fullness, 5, 5);
...
bool Pet::restoreCareSnapshot(const CareSnapshot &o, uint32_t nowEpoch) {
  ...
  if (!(o.speciesId == -1 || (o.speciesId >= 1 && o.speciesId <= DEX_COUNT))) return false;
  ...
  for (uint32_t i = 0; i < mins; i++) {
    ...
    if (weight > 50 && ageMinutes % 3 == 0) energy = dropTo(energy, 1, 20);
    if (weight > 0 && ageMinutes % 3 == 0) weight--;
  }
  if (!isEgg() && awakeMins) {
